...
The goal of this lab is to get you started with interrupts in preparation for using in your fullsystem.

During this lab you will:

- Review the provided code for managing the interrupt system,
...
1. Be up to date on [recent lecture content](/schedule/#Week7): __Interrupts__
    - Review the interrupt support code we introduced in lecture (all source files in directory `$CS107E/src`);
        - `interrupts.c` library module to manage trap_handler, interrupt sources, and dispatch to handler per-source
        - `gpio_interrupt.c` library module to manage GPIO interrupts and dispatch to handler per-pin
...
1. Organize your supplies to bring to lab
    ...
    - You will need a HDMI monitor; we have monitors available in lab for shared use.
...
#### 1) Review interrupts module

In the first lecture on interrupts, we went over the low-level mechanisms. The `interrupts` module configures the platform-level interrupt controller. The module interface is documented in [interrupts.h](/header#interrupts) and its implementation is in [interrupts.c](/src#interrupts). The source and headers files are available in directory `$CS107E`.

Hardware support for machine-mode interrupts is via CSRs such as `mtvec`, `mepc`, `mstatus`, etc. CSRs can only be accessed via special `csr` assembly instructions. The header file [csr.h](/header#csr) defines `CSR_xxxx` macros that use inline assembly to access those special instructions. Here are some of the CSRs used in the interrupts implementation:

-  `interrupts_init`
    - `CSR_WRITE(mtvec, ...)` installs function pointer that will be called to service a trap
- `interrupts_global_disable` and `interrupts_global_enable`
    - turn on/off the top-level interrupts switch by setting bit in `mstatus` CSR
-  `_trap_handler`
    - Accesses CSR values for the current exception such as `CSR_READ(mcause)` and `CSR_READ(mepec)`

The trap function should be declared `__attribute__((interrupt("machine")))`. Review the [gcc documentation on function attributes](https://gcc.gnu.org/onlinedocs/gcc/RISC-V-Function-Attributes.html) to learn more. The `interrupt` attribute changes the function's assembly to include a prolog and epilog that safely enter and exit interrupt processing.
...
#### 2) Review dispatch for interrupts and gpio_interrupts

Control flow on an exception jumps to the `_trap_handler` function defined in `interrupts.c`. If the trap cause was an external interrupt, `_trap_handler` dispatches to the handler function set for the interrupt source. All other trap causes are an unrecoverable fatal exception, which is handled by printing an error message and calling `mango_abort` to halt execution

In our second lecture on interrupts, we reviewed the design for dispatching events to the correct handler. The dispatch operation in both `interrupts` and `gpio_interrupt` uses an array of function pointers, one per-index. The client writes a handler function and registers it with the dispatcher. The dispatcher stores the client's function pointer in the array at the associated index. When an event occurs, the dispatch invokes the handler function at that index. In the top-level `interrupts` module, the interrupt source number is the index into the array of handlers. In the `gpio_interrupt` module, the pin index within the gpio group is used as the index.

There is a neat performance trick that applies in the `gpio_interrupt` version. To identify which gpio pin has a pending event, the dispatcher scans the status registers to find the first bit that is set. If implementing the scan in C, you might loop and shift/test each bit individually. A more streamlined version could do fancy bit twiddling, such as [Kernighan's bit count](https://stackoverflow.com/questions/12380478) algorithm, or employ on a lookup table. Such versions might take tens or even hundreds of cycles.
...
+ How is a function set as a handler with a dispatcher? How does the dispatcher know which handler to call for a given event? Can there be multiple handlers set for the same event? If there is no handler set for a pending event, what will the dispatcher do with the event?
...
This circuit looks like the PB4 will be floating (unconnected) in default state when the button is unpressed. Back in Assignment 2, we corrected this by adding a hardware pull-up resistor. Now we use code to activate the internal pull-up resistor instead. When unpressed, the button will read high. When pressed, it closes the circuit and button will read as 0.
...
+ Wait for a falling edge on the button gpio, i.e. repeatedly call `gpio_read()` until observe a transition from 1 to 0. Revisit your assign5 code `ps2_read` if you need a refresher on what loops are needed.
+ After observe falling edge, `wait_for_click` should increment count and print a message.

Compile and run the program. The program sits waiting for a button click. Click the button, and it prints a message and redraws the screen. This version of the program is either redrawing OR waiting for a click. While waiting for a button press, the screen redraw is paused. While redrawing the screen, no button presses are detected. Ideally, we want the program to do both tasks concurrently.
...
- Extend `config_button()` to add a gpio interrupt for button event following the steps below:
    ...
    + call `gpio_interrupt_set_handler()` to register your handler function
...
- Edit `config_button` to take one argument, the `rb` pointer. When calling `gpio_interrupt_set_handler`, pass the `rb` pointer as `aux_data.`
...
- Edit `main` to redraw once for each update. Delete the previous code that compared `gCount` to `drawn` Instead, check if the ring buffer is non-empty and if so, call `rb_dequeue` to retrieve the next update and redraw for it.
...
### Project brainstorm and partner speed-dating
...
the project next week, but we'd like to get y'all talking to one another about
your project interests and what you're looking for in a project partner. Project teams
are generally pairs. If you have a project in mind that seems better scoped
for a single or trio, let's chat about suitability to make exception.
