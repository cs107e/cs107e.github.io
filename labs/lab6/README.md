...
Change to your local `mycode` repo and pull the lab starter code:
...
Partner up with your neighbor(s) to share one monitor for the rest of the lab exercises.
...
and `hdmi` modules.
Peruse the module interfaces [$CS107E/include/de.h](/header#de) and [$CS107E/include/hdmi.h](/header#hdmi). These modules export
very few functions that are present to support exactly and only the specific features needed for our system.
...
You will need access to an HDMI monitor for assignments 6 and 7. You are welcome/encouraged to use the shared monitors in lab. We have vetted the compatibility of these monitors with libmango and made them easy to connect by perma-attaching a mini-to-standard HDMI cable. __Our monitors and attached cables must stay in lab for all to use.__  To work elsewhere, you will need to track down a HDMI computer monitor, tv, or projector and a mini-to-standard HDMI cable/adapter.
