...
1. Be up to date on [recent lecture content](/schedule/#Week4): __Stack/heap and linking__
    - Give special attention to [recap of stack frames](/lectures/Memory/stack_notes) in readings for Memory lecture.
...
32   int main(void) {
(gdb) # commands to view main function C code and generated assembly
(gdb) list main
(gdb) info address main
(gdb) print $pc
(gdb) disassemble
(gdb) disassemble 0x40000000,0x400001e8
(gdb) # Examine memory: x/FMT ADDRESS. ADDRESS 0x40000000, FMT 2 words in hex, 8 bytes or 2 instruction
(gdb) x/2wx 0x40000000
(gdb) x/8bx 0x40000000
(gdb) x/2i  0x40000000
(gdb) # Examine memory at current stack top, FMT 4 words in hex or as 2 giant words
(gdb) print $sp
(gdb) x/4wx $sp
(gdb) x/2gx $sp
...
- Use `disassemble` arguments to disassemble a function by name or range of addresses. Look at the full memory diagram and find addresses `0x40000000` to `0x400001e8`. This is the "text" section, containing binary-encoded instructions. In gdb, use `disassemble 0x40000000,0x400001e8` to see the section contents. The instructions should match what is shown in the full memory diagram.
...
The gdb `display` command adds an expression to be auto-printed after each step. The useful display command `display/i $pc` auto-prints the `$pc`. As you single-step, this auto-print expression will display the instruction that is next to be executed.

```console?prompt=(gdb)
(gdb) display/i $pc
=> 0x400001b8 <main>:   add sp,sp,-16
```
Use `disassemble` to view the assembly instructions for `main` and try out `stepi`.
...
(gdb) # now stepi through a few instructions
(gdb) stepi
...
- A reciprocal set of instructions at the end of the function deconstruct the frame. These instructions are called the function _epilog_.
    - The prolog/epilog follow a standard format with minor differences for which registers are saved and amount of space set aside for local variables.
...
1. `add sp,sp,-16`
    ...
    - The RISC-V ABI requires the stack pointer to always be aligned to a multiple of 16, so the adjustment may include padding (e.g. if only 20 bytes were needed, it would adjust the stack pointer by 32 to meet the alignment rule).
2. `sd  ra,8(sp)` and `sd fp/s0,0(sp)`
    - These next two instructions save the values of the `ra` and `fp` registers to the stack.
    - The `sd` instruction stores a register value to memory. The `d` is for "double-word" which is 64-bits.
    - Register `x8` has two nicknames: `s0` and `fp` that are synonymous. The gdb disassembly uses `s0` ; I wish it instead used `fp` to remind of the register's purpose, sigh. We will refer to it as `fp/s0` to keep two names connected in your mind.
    -  The first 16 bytes of a RISC-V stack frame are used to save registers `ra` and `fp/s0`. The `saved ra` is pushed first and underneath is the `saved fp/s0`.
        - `saved ra`
            - What is the `ra` register used for? What will be the value of the `ra` register when starting the function `main`?
           - The `ra` is an address in the text segment. In your stack diagram, write in that value and draw an arrow that points to that address. Which function is this address contained in?
           - Why does the stack frame store a saved copy of `ra`? How can you use the `saved ra` to identify the caller of this function?
        - `saved fp/s0`
            - The `fp/s0` is an address in the stack. Where in the stack does this address point to?
            - Why is it useful for the stack rame to store a saved copy of `fp/s0`? How can this be used as part of a backtrace operation?
3. `add s0,sp,16`
...
The prolog is now finished and execution continues with instructions in the body of the `main` function. The next few instructions set values for the parameters and make a call to the function `combine`. Keep single-stepping with `stepi` until execution enters the `combine` function. Use `disassemble combine` to view its assembly instruction and note that it starts with the same function prolog instructions as `main` did. Trace/single-step through its prolog, and extend your stack diagram to add the stack frame for `combine`.

Having traced the prolog twice, you are starting to see a stack frame is set up. Each function does the same three steps: adjust the stack pointer to make space, store `saved ra` and `saved fp/s0` to the stack (along with other saved registers), and anchor the `fp/s0` for this frame.

When completed, your stack diagram should show the data on the stack after entering the `combine` function. Compare your stack diagram with your tablemates and ask questions of each other and resolve any discrepancies Confirm your answers with ours: [stopped in combine](images/stopped-combine.html). We want everyone to have a rock solid understanding of how a stack frame is laid out and what each of the values mean. Check in with us to confirm your understanding. [^1]
...
- If the code erroneously wrote to an index past the end of this stack-allocated array, what data would be corrupted?  What kind of consequence will result from this error?
- If the code erroneously returned the address of a stack-allocated array, what will happen when the caller accesses the memory at that address?

Writing outside the bounds of a stack-allocated array can be a devastating error. The array is located in stack memory near critical values such as the `saved fp/s0` and `saved ra`. Writing past the array can overwrite these precious saved values which then causes execution to "teleport" to a completely nonsensical place. Even writing a single extra byte is too far if the array immediately abuts a critical saved value. Ouch! The effect of stack corruption can be a baffling phenomenon the first time you encounter it. Knowing how the stack housekeeping is managed provides helpful insights for recognizing the situation and debugging it.
...
([Stack diagram sheet](images/stack_diagrams_to_print.pdf) to print for students)
...
[^3]: How do symbol addresses in a `.o` file (pre-link) differ from `.elf` file (post-link)? Use `nm` to compare. What is the significance of lowercase versus uppercase type letter in the nm output?
[^4]: Explain the difference in a build error from forgetting to `#include "module.h"` versus forgetting to link against `module.o`.
