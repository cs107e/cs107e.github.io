...
1. Watch this excellent tutorial video (20 min) from Ben Eater <https://www.youtube.com/watch?v=PE-_rJqvDhQ> on best practices for breadboarding. Ben is the 🐐!
...
## Lab exercises
The lab has four exercises. During the lab session, briskly move through the first three exercises (~20 minutes each) in the first hour, reserving the second hour for breadboarding exercise 4. You will not complete the entire breadboard, but an hour is enough time to get a solid start and leave lab with a clear understanding of how to finish on your own.

You will have to move pretty quickly on the first three exercises, so plan to circle back for a fuller exploration when you have time. If you run into any issues or have follow-up questions, post on Ed or come by office hours to talk with us about it!
...
$ git pull code-mirror lab2-starter   # pull starter code for this lab
```

### 1. C to assembly (20 min)

Compilers are truly an engineering marvel. Translating C source into an appropriate sequence of assembly instructions with correct use of registers and memory is both a technical and artistic achievement. From here forward in the course, you'll hand over the task of writing assembly to `gcc`, but you will continue to grow your reading fluency of assembly. Use this exercise to observe and pay tribute to the handiwork of the compiler.

Sometimes the assembly
produced by the C compiler can be surprising. You will be pleased by the clever optimizations it provides, but need to be aware that its eagerness to streamline can sometimes remove or rearrange your code in ways that confound your intentions. When this happens to you, you can deploy your assembly superpowers to dig into the generated
...
In the file `codegen.c`, find the section marked `Part (a): arithmetic`. Paste the functions one by one into the Compiler Explorer source pane on the left and review the generated assembly shown on the right. Confirm you understand how the assembly accomplishes what was asked for in the C source. Do you note any surprising choices in how it goes about it?  Read our comments in `codegen.c` as the guide for what to look for and what follow-up experiments to try.
...
The final part includes some C code for which the generated assembly is bit shocking -- examine those closely to understand what is happening and why. When C code ventures outside the boundary of legal C, it is said to have "undefined behavior" and a compiler is allowed to completely disregard it.  Check out this article for more examples <https://embeff.com/compiler-dependent-behaviour-in-practice/>
...
#### Add target `make run`
You can include targets in a Makefile to automate other tasks, such as to execute your newly-built program on the Pi.  Open `Makefile` in your editor and add a new target `run` to invoke `mango-run` on the program binary.  Be sure to list the file `blink_actled.bin` as a dependency, so run will first rebuild if needed. Try out your new target:
```console
$ make run                # find run target in Makefile and do recipe
```
...
### 3. Testing (20 min)
<a href="testing"></a>
An effective developer knows testing goes hand-in-hand with writing code. The better your tests and more timely you use them, the sooner you will find your bugs and the easier your debugging will be. To help you grow this important skill, your assignments will incorporate a required testing component.

In CS106B, you constructed test cases using C++ SimpleTest `STUDENT_TEST` and `EXPECT`. The testing strategies you were using will still serve you well, but the testing framework is less fancy. The standard C library includes an `assert` macro for use as a simple diagnostic.
```console
$ man assert         # see man page for assert in standard library
```
You apply the `assert` macro to an expression that must be true. If the expression does evaluate to true, the assertion succeeds and the program continues on.  If the expression is false, the assertion fails which cause the program to print an error message and exit.
...
#### A buggy program
Let's walk through an example that shows using `assert` as rudimentary testing tool.
...
- if the program executes a single test case of `count_bits` on a value that has a bug?
- if the program executes a single test case of `count_bits` on a value that works correctly?
- if the program executes several `count_bits` test cases, some which pass and some which fail?
...
This Makefile has a `test` target that builds the test program and executes it on the Pi. Try it now:
```console
$ make test         # build and run test program on Pi
```
The program runs and the blue ACT LED starts blinking. Hmmm, that means at least one test failed, but which one?
...
You're ready to check-in with us about unit tests.[^3]
...
The 7-segment display, as its name implies, is comprised of 7 individually lightable LEDs, labeled A, B, C, D, E, F, and G. There is also a decimal point labeled DP, which we will not be using. Each segment is an LED. Recall that an LED has an anode and a cathode and polarity matters: the anode voltage must be positive relative to the cathode to turn on the LED.
...
To show a digit, you create a circuit by connecting the
common cathode to ground and applying a voltage to the desired segments. Turning on all seven segments would show the digit `8`.
...
#### Datasheets
Here is the [datasheet for the 5641-AS](/readings/7segment_5641AS.pdf) display unit. A datasheet is your go-to when learning a new component. It will lay out the specifications, schematic, pinout diagram, operational behavior, testing data, and more. Datasheets vary a lot in how well they present the information, some are organized and direct, otherwise barely readable.  Learning to read a datasheet found in the wild is a good skill to work at developing. In our writeup below, we extracted the essential facts from the datasheet to get you started.
...
Untangling the connections can be a bit tricky. There are twelve pins in total: four digit pins (`DIG.1`, `DIG.2`, `DIG.3`, and `DIG.4`) and eight segment pins (`A`, `B`, `C`, `D`, `E`, `F`, `G`, `DP`). Each segment pin connects to all four digits; start at the pin numbered 11 and follow its connections to the `A` segment of each digit. Each digit has its own unique ground, e.g. `DIG.1` is the cathode/ground pin for digit 1. Trace how each segment connects to the shared digit grounds.
...
How do you know if your breadboard has a mid-rail gap? Check whether rail marking is a continuous line or has a break at mid-rail. The breadboards in the photo below have a mid-rail gap. Click the image to zoom in and see the markings. The rest of the photos in this lab writeup use a breadboard that does not have a gap, so may look different than yours.
...
Take note that each hole on the wire strip corresponds to a specific wire gauge. Breadboarding wire is typically 22 AWG (or sometimes 20 or 24 AWG). Read the label on the spool to determine the gauge of the wire you are using and identify the hole on the stripper that matches your wire gauge. When you place your wire into the correct gauge hole, the stripper jaws will pull off just the casing, leaving the wire core intact.
...
Each LED in the display unit requires a current-limiting resistor just as a single LED does. Place a 1K resistor bridging the middle of the breadboard to the right of the display unit. The resistor should sit neatly. In the video below, Liana shows using the pliers to make a sharp crease and clipping the leads with the cutter to make a nice secure fit.

<video controls="controls" width="500" name="fitting resistor" src="images/fitting_resistor.mp4"></video>


Now you will make some temporary connections to the display unit to test turning on a single segment. While testing, make connections using male-male jumpers. After validating your circuit, go back and re-wire it in a neater and more permanent fashion.
...
- Add jumpers from the Mango Pi header to your breadboard power and ground rails.
    - Have your Pi disconnected from power while changing the wiring.
    - Pick out a pair of red and black male-female jumpers.
	- Use the Mango Pi pinout to identify 3.3V and ground pins on the Pi's header.
	- Use the red jumper to connect from 3.3V header pin to the power rail (red) of the breadboard and the black to connect from a ground header pin to the ground rail (black or blue).
...
You can change which segment and digit is lit by moving the jumpers to different pins on the display unit. Try moving the black jumper from digit 1 to digit 2. Segment B of digit 1 turns off and segment B of digit 2 turns on. If you add a second black jumper that grounds digit 3, now segment B will light on __both__ digits.  Note
...
So far, we are controlling whether a digit is on by adding or
removing a jumper that connects the digit pin to ground, i.e. changing the hardware.
In order to instead use software to programmatically control which segments and digits are turned on,
we need an electronic switch that can be controlled by a GPIO pin.
...
__Don't forget to clean up!__ Please use the hand vac or brush/dustpan to clean debris from table and floor and return shared tools and supplies to their rightful place.  We all appreciate having an organized and tidy workplace. Thank you!
{: .callout-warning}

<div class=checkinsheet markdown="1">

<img src="images/refsheet_to_print.png" width="1224">
<div class="new-page">
</div>
...
[^3]:  Do you have the general gist of using `assert` for test cases? You'll need for this assign2 unit tests, so be sure to ask if something not clear.
[^4]:  Show us your display unit directly wired and lit up to show `"1 1 "` (and again with transistors if you get there).
[^5]:  Before leaving lab, confirm you have a solid start on the breadboard and a clear understanding of how to complete the remaining tasks on your own. Do you need followup assistance? How can we help?
