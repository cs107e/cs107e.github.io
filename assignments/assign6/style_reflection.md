...
1. One challenge when designing a module is striking a good balance between building in flexibility for anticipated future use cases versus a simpler design that fits exactly and only what is needed today. Consider the assign 5 `keyboard` module. Tell us what code changes would be needed to support a different keymap like Dvorak. How much work would be required to add support for other modifiers, e.g alt-letter or combos of existing modifiers? Identify an extended feature the current design does not seem to anticipate and how you would change the design to better accommodate it.
