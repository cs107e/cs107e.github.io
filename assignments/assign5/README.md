...
cs107e rocks my sox 
Pi> echo I can use poke to turn on the blue LED!
I can use poke to turn on the blue LED!
Pi> 0x20000a0 0x40000
</pre>
...
uart to a Mac laptop running minicom.
...
The `clear` command sends a formfeed `\f` character to your terminal. For programs that support it (such as minicom), this will clear
the screen and position the cursor in the top row. (If your terminal doesn't support formfeed, `clear` is a no-op. _C'est la vie..._)
...
Pi> poke 0x20000a0 0
  ...
  __Tread carefully when testing peek and poke__. The `peek` and `poke` commands are operating on live memory.  Valid addresses to peek are those within the range of the Mango Pi DRAM (0x40000000 - 0x7fffffff) and addresses of memory-mapped peripherals such as gpio and uart. Accessing addresses outside the known memory map can behave unpredictably. Indiscriminant use of `poke` can be quite deadly as you are changing values in memory out from under the executing program.  Before you `poke` an address, be sure you know what is stored there and what effect your change will have. Once you have tested `peek` and `poke` on a few carefully selected addresses, it is reasonable to extrapolate that it will also handle other valid locations without testing on every single address in the entire address space. 😅
...
What other bonus features would supercharge your shell and make it your own? Our reference shell throws in a few cute add-ons such as `gpio` (to control the gpio pins), `calc` (very simple calculator),and  `hexdump` (extended peek). If you wrote the disassemble extension for assign3, a nifty option is to integrate a `disassemble [addr]` command into your shell. What about adding `tab` to auto-complete command names to save your overworked fingers? The sky's the limit!
