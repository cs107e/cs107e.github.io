...
```
 %c   single character
 %s   string
 %d   signed decimal integer, %ld long, optional width zero-pads
 %x   unsigned hexadecimal integer, %lx long, optional width zero-pads
 %p   pointer
...
- __Use the debugger__! Run under gdb in simulation mode and use gdb commands to
  step and print variables to observe your program's operation. We strongly encourage you to invest in building up your gdb chops now -- this investment will really pay off! Stay mindful of the differences between the simulator and the actual Pi. (Review [Lab 3](/labs/lab3/) for a refresher)
<a name="gdbinit">
   ...
   Auto-loading commands from CS107E/other/gdbsim.commands...
    ```
    * If the program you are debugging attempts to access the uart peripheral, be sure gdb is reading the `gdbsim.commands` configuration file we provide.  Once you are inside gdb, confirm that it has been auto-loaded by looking for our special breakpoint:
    ```console?prompt=(gdb),#
    (gdb) info break
