...
Completing this assignment will give you two new modules to add to your growing library of functionality for your Mango Pi.
...
1. **Test as you go**. Spending 10 minutes upfront to write a test
...
int unsigned_to_base(char *buf, size_t bufsize,
                     unsigned long val, int base, size_t min_width);
int signed_to_base(char *buf, size_t bufsize,
                   long val, int base, size_t min_width);
 ```

These helper functions convert the long integer `val` to a string representation in the specific base `base` and write the characters of the output string to the memory location pointed to by `buf`.
...
The `val` argument is the value to be converted. If `signed_to_base` is called with a negative value, the output string will contain a leading minus sign.
...
```
 %c   single character
 %s   string
 %d   signed decimal integer (%ld long decimal), optional width
 %x   unsigned hexadecimal integer (%lx long hex), optional width
 %p   pointer
...
- __Use the debugger__! Run under gdb in simulation mode and use gdb commands to
  step and print variables to observe your program's operation. We strongly encourage you to invest in building up your gdb chops now -- this investment will really pay off! Stay mindful of the differences between the simulator and the actual Pi. (Review gdb exercises of [Lab 3](/labs/lab3/) for a refresher)
<a name="gdbinit">
    * Use the `make debug` target from the Makefile to run the debugger on the test program `.elf` version of the program you wish to debug.
    ```console?
   $ make debug
   ...
   Auto-loading commands from CS107E/other/gdbsim.commands...
    ```
    * If the program you are debugging attempts to access the uart peripheral, be sure gdb is reading the `gdbsim.commands` configuration file we provide.  Once you are inside gdb, confirm that these commands have properly loaded by looking for our special breakpoint:
    ```console?prompt=(gdb),#
    (gdb) info break
    Num     Type           Disp Enb Address            What
    1       breakpoint     keep y   0x0000000040000f84 in uart_putchar at reference/uart.c:185
            silent
            printf "%c", $a0
            return
            cont
    ...
    > __Does your gdb get stuck when your program tries to print output?__  The gdb simulator does not emulate the peripherals (no gpio, no timer, no uart, etc.) so a program that attempts to use the uart will be DOA without the help of our `gdbsim.commands` file. Re-read the instructions above.
    {: .callout-warning}
...
![instruction encodings](images/encodings.png)
...
(gdb) x/i &add
0x4fffffdc: add a3,a4,a5
(gdb) disassemble main
Dump of assembler code for function main:
   0x0000000040000aac <+0>: add sp,sp,-32
   0x0000000040000ab0 <+4>: sd  ra,24(sp)
   0x0000000040000ab4 <+8>: sd  s0,16(sp)
   0x0000000040000ab8 <+12>:  jal 0x40001d90 <uart_init>
```
