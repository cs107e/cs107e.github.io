...
since printing program state is a valuable form of debugging. The C library function to output formatted text is `printf`. The standard version of `printf` has extensive features and a large code size.  You will implement a
...
### Strings module
Pretty much every programming language supports a string data type and operations. The C language's string type is very bare bones: a pointer to a contiguous sequence of characters terminated by a null-terminator (zero, or `'\0'`). You can access individual characters using pointer or array operations and that's about it. To do more useful  things with a C-string, such as find its length, make a copy, or compare two strings, you need additional functions. The standard C library implements a lot of string functions!
```console
$ man string                  # documentation for string functions in standard library
```
...
The standard `snprintf` (as documented in its man page `man snprintf`) is a full-featured workhorse with an extensive set of options. Our libmango version supports a selectively chosen set of specific uses. Refer to our [printf.h](/header#printf) header file to know exactly what your code is required to support.
...
- learn more about standard library functions <https://en.cppreference.com/c/string/byte> and implement additions
    ...
    - what is the difference between `memmove` and `memcpy` and why do we need both?
    - check into `ctype.h` operations that classify and convert characters -- neat how ASCII values permit using numeric operations to implement these operations.
    - what is purpose of `restrict` qualifier placed on pointer arguments?
    - libmango has (non-standard) `strlcat` but not `strncat`. What is the difference and why do you think we made that choice?
    - what kind of inputs result in undefined behavior by the standard string functions? why do you think the spec is written that way?
- additional bases
    - if you designed your printf with clean unified handling of hex/decimal, adding bases for octal `%o` and binary `%b`(custom/non-standard) can be a snap.
- extend support for field width, as in standard printf
    - negative field width to left-justify, pad on right: `printf("%-5d", 42)`
    - field width specified by argument: `printf("%*d", 5, 42)`
- learn about IEEE floating point format and how to convert double value to string
    - cheezy hack can kinda work, scale up to get decimal places, output integer `whole.fraction`
    - more interesting to learn about float representation and practice bit dissection
    - translating float bits to hexadecimal float `%a` is somewhat direct (but output is kind of inscrutable)
    - `%e` and `%f` conversions are much more involved, but output much nicer
        - maybe focus on handle certain values (say, exponent = 2^0) as proof of concept?
- support color output
    -  ANSI escapes into uart output to control text color/style (tio terminal will receive and interpret)
    - <https://en.wikipedia.org/wiki/ANSI_escape_code>
- invent your own formatting codes and add to your printf
    - money format to print double value as `$1,510.99`? date and time formats?
- make disassembler
    - add custom code `%pI` to output human-readable assembly for a binary-encoded instruction, pass address of instruction as argument (pointee type `uint32_t` to access 32-bit binary-encoded instruction in memory)
    - this is a great way to learn more about RISC-V, instruction encoding, PC-relative addressing, signed/unsigned representation, bitmasking, and more!
        - look up `bitfields` in a C reference to learn a super neat way to access the instruction bits
    - full disassembler would be a big job, you can learn a lot from working up just a few
        - try decode an opcode or two within each of the four types (R-type, I-type, SB-type, UJ-type)
        - once you work up decode one opcode for a type, filling out other opcodes is mostly repetition, not so much to learn
    - see [RISC-V ISA Manual](/readings/riscv-spec-20191213.pdf#page=34) for documentation on instruction encoding. Neat [online encoder/decoder](https://luplab.gitlab.io/rvcodecjs/#q=fe010113&abi=true&isa=RV64I) is fun way to learn more!
- implement `sscanf` for parsing formatted input
    - `man sscanf`  <https://en.cppreference.com/c/io/fscanf>
    - inverse of printf: conversions extract values from text and write to arguments (passed by reference)
    - use with uart to build simple input functions such as `get_line`, `get_integer`, etc.
- OR something else entirely, come by office hours to brainstorm your ideas with us

Edit `extension.c` for your extension program (may be either application or unit tests for extended features) and use `make extension` to compile and run. Use the same git workflow as core, i.e. edit on dev branch, regularly commit and push. Edit the `README.md` file with quick summary of what you did for the extension. Apply tag `assign3-extension` to submit when done.
...
- all `assign3-submit` files commit/tag/push by due date or before end of grace period

Additional for `assign3-extension`:

- extension program `extension.c`
- `README.md` with information on your extension
- use tag `assign3-extension`, commit/tag/push files by end of grace period


> __Revise and retest__  If our tests find a bug in your `assign3-submit`, we post an issue to your GitHub repo. These open issues become a worklist for you to follow up on and resolve. Once you have prepared your bug fix for retest, commit your changes, tag `assign3-retest` and push.  On the next retest run, we'll update your issues to show new successes.  Retests are accepted up until the full system deadline (~week 7-8).
This flexible retest policy means it is okay to make an original submit that is incomplete or has known bugs (a "first draft" if you will), knowing that you will be able to circle back and fix things up on the retest timeline. What matters most is that your full system is complete and correct at the end.
{: .callout-info .small}
