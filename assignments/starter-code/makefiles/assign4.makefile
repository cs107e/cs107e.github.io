...
# Paths to binaries.
APPLICATION = build/bin/heapclient.bin
TEST 		= build/bin/test_backtrace_malloc.bin
...
build/obj/nameless.o: CFLAGS += -mno-poke-function-name

# Create build directory and its subdirectories.
build:
	mkdir -p build/obj build/elf build/bin build/list
	
# build the disassembler extension
disassemble: disassemble.elf disassemble.list disassemble.bin | build
	rpi-install.py -p build/bin/disassemble.bin
