...
static void show_rainbow(int offset) {
    int nleds = 16;
    color_t colors[] = {GL_RED, GL_ORANGE, GL_YELLOW, GL_GREEN, GL_BLUE, GL_INDIGO, GL_PURPLE};
    int ncolors = sizeof(colors)/sizeof(*colors);

    for (int i = 0; i < nleds; i++) {
        send_pixel_color(colors[(i+offset) % ncolors]);
    }
    // after send entire sequence, hold data low for pixels to latch and display
...
void main(void)  {
    ...
    for (int j = 0;  ;j++) {
        show_rainbow(j);
        timer_delay_ms(100); // delay for latch (edit for faster/slower frame rate)
    }
}
