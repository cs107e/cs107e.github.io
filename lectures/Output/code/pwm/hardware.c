 ...
 * Using pwm module from libextra
 *
 * Set color components of common anode RGB led
 * Square wave audio output to passive buzzer
 */
#include "gpio.h"
#include "gl.h"
#include "pwm.h"
...
void main(void) {
    pwm_init();
    pwm_config_channel(BLUE.ch, BLUE.pin, 75, PWM_ACTIVE_LOW);
    pwm_config_channel(RED.ch, RED.pin, 75, PWM_ACTIVE_LOW);
    pwm_config_channel(GREEN.ch, GREEN.pin, 75, PWM_ACTIVE_LOW);
    pwm_config_channel(AUDIO.ch, AUDIO.pin, 1200000, PWM_ACTIVE_HIGH);
    play_scale();
    timer_delay(1);
    sound_effects();
}
