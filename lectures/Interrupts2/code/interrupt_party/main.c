...
static void button_click(uintptr_t pc, void *aux_data) {
    gpio_interrupt_clear(BUTTON);
    last_button = elapsed;
    nbutton++;
}

static void second_elapsed(uintptr_t pc, void *aux_data) {
    hstimer_interrupt_clear(HSTIMER0);
    elapsed++;
}

static void clock_edge(uintptr_t pc, void *aux_data) {
