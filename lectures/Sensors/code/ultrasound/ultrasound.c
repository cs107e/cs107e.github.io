...
static int sense_distance(gpio_id_t trigger, gpio_id_t echo) {
	...
	timer_delay_us(10);    // pulse (hold trigger higgh for 10 us)
...
static void show_reading(int inches) {
...
void main(void) {
    const gpio_id_t echo = GPIO_PB4, trigger = GPIO_PB3; // echo-blue trigger-yellow
