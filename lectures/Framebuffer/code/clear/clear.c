...
static void clear_char_by_char(void) {
    unsigned char *im = module.framebuffer;
...
static void clear_int_by_int(void) {
...
void main(void) {
    ...
    while (1) {
        ...
        if (!confirm("clear to blue, int by int")) break;
        clear_int_by_int();

        if (!confirm("clear to red, 2-D of int")) break;
        clear_2d();
    }
    printf("Completed %s\n", __FILE__);
}
