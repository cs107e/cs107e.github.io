...
void main(void) {
    ...
    if ((mp = property_get(TAG_GET_ARM_MEMORY))) {
        printf("Arm Memory: Base address: 0x%08x, Size in bytes: 0x%08x (%dMB)\r\n",
           mp->data.buffer_32[0], mp->data.buffer_32[1],
           mp->data.buffer_32[1] / (1024 * 1024));
    }

    if ((mp = property_get(TAG_GET_VC_MEMORY))) {
        printf("GPU Memory: Base address: 0x%08x, Size in bytes: 0x%08x (%dMB)\r\n",
            mp->data.buffer_32[0], mp->data.buffer_32[1],
            mp->data.buffer_32[1] / (1024 * 1024));
    }
