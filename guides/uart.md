...
<A name="find-dev"></A>
## Find device name
When the usb-serial is plugged into your laptop, it appears as a new entry in the filesystem, with a name of the form `/dev/YOUR_DEVICE_NAME` for some value of __YOUR_DEVICE_NAME__. You'll need to know the device name to connect to it.
...
1.  Instead apply some command-line superpowers to suss out the device name.
    The idea is to capture a listing of the files in directory `/dev/tty*` when the usb-serial is unplugged and take another capture after connecting it. Applying `diff` to the two listings will show what changed, which gives you the device name for the usb-serial. Knowing your way around the unix command-line can be very helpful!
...
2. Use three jumpers to connect transmit, receive, and ground between the usb-serial and the Pi
    ...
    We recommend the arrangement shown above: OTG connected to end of hub and usb-serial connected to side of hub (this configuration leaves open access to the Pi's other USB/HDMI ports along the long side).

>__Review connections__ The connections run from one device's TX to the other's RX, i.e. the data __transmitted__ by your laptop is __received__ by your Pi and vice versa. Connecting TX to TX and RX to RX is not correct!
{: .callout-warning}

The usb-serial in your parts kit may be a bit different than photos above, so be sure to read the labels on your part rather than follow the photos blindly. Here are a few examples of usb-serial variants we have used (click to enlarge):
...
There are a number of programs you can use to communication with a tty device (`screen`, `minicom`, `putty`, etc.) Our favorite of the bunch is `tio` for its simple interface and nice features, so this is our strong recommendation. (If you already very comfortable with another program, ok to keep using instead).

> __Got tio?__  See guide on how to [install and configure tio](/guides/install/tio).
{: .callout-warning}


A successful uart connection requires that the transmitter and receiver agree on all settings: device name, baud rate, use of start, parity, and stop bits. We use these settings between your laptop and Mango Pi:
- peripheral `UART0`
...
- If tio gets a "permission denied" error when trying to connect to your device, try adding your user id to the group of the `/dev` files (possibly `tty` or `dialout`). Ask a staff member if you need help.
