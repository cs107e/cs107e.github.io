...
We will use USB to supply power to the Mango Pi. Find the two USB-C ports along the edge of the Mango Pi board. The port that is closer to the corner is labeled `OTG`.
USB On-The-Go (OTG) is a USB specification that allows devices to switch between the roles of host and device. Connecting a 5V power source to either usb port will power up the Pi, but we connect to the OTG port because it additionally suppports the communication protocol used by the [xfel bootloader](/guides/xfel) we will be using to load and execute programs on the Pi.
...
The total power being consumed is 0.41 W. That is not a lot of power! The power may go up if the Pi is driving LEDs and other peripherals.
...
The latest (Dec 2025) rev of the Mango Pi board __includes a reset button__.  Look for the tiny push button on the edge of the board between hdmi and USB ports. You can reset the Pi by clicking this button, yeah! 🔥
