...
### On Windows: 
Hit start and open up windows powershell, enter the following command
```console
$ Get-CimInstance -Class Win32_SerialPort | Select-Object Name
```
...
NOTE: When you use `minicom` to connect to your pi, you will need a device name. 
You can find that device name by looking at which COM port you have, i.e COM3 translates to `/dev/ttyS3` , COM1 : `/dev/ttyS1`


### On Linux or Mac 
Linux has `lsub` which you might prefer but the following method also works 

With your CP2102 plugged in, run 
```console
ls /dev > in.txt
```
then unplug your CP2102 and run 
```console
ls /dev > out.txt
```

Now you have a two lists, one of all your usb devices when your CP2102 is plugged in, one when it was unplugged
So we can now diff them to see our device 
```console
diff in.txt out.txt 
```
...
If you do not see anything from this step your drivers likely did not install correctly, grab a TA for some help!
