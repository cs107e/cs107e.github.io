 ...
 * process the event. A handler takes one argument of type unsigned int
 ...
 *
 * Attach a handler function to a given interrupt source. Attaching a
 ...
 *
 * The interrupt source identified by number. Valid interrupt sources are 
 * listed below in the enumerations `interrupt_source` and `interrupt_basic`. 
 ...
 * When an interrupt is received, each attached handler for that source
 * is called. It is the responsibility of a handler to check whether 
 * the event is of the specific sort this handler processes.
 * If so, the handler should process and clear the interrupt and
